...
async function loadEditCampaignData() {
  ...
  try {
    ...
    if (startPicker && startDateObj && !isNaN(startDateObj.getTime())) {
      // In edit mode, allow old start date to show even if it is before today
      startPicker.set("minDate", null);
      startPicker.setDate(startDateObj, true);
    } else if (startDateInput && startDateObj && !isNaN(startDateObj.getTime())) {
      startDateInput.value = formatDateForFlatpickr(activity.start_date);
    }

    if (endPicker && endDateObj && !isNaN(endDateObj.getTime())) {
      if (startDateObj && !isNaN(startDateObj.getTime())) {
        endPicker.set("minDate", startDateObj);
      }

      endPicker.setDate(endDateObj, true);
    } else if (endDateInput && endDateObj && !isNaN(endDateObj.getTime())) {
